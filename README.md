...
入口先把左臂恢复到运输安全初始位，然后将底盘向右移动 `2.0 m`。长距离移动被拆成最多 `0.08 m` 的短步；每一步都必须同时获得新鲜里程计、前后双雷达和底盘速度控制租约，任一条件失效即发送零速并终止全流程。之后依次执行黑色底座/灰色导热片闭环定位、记录抓取位闭爪、垂直上提 `0.12 m`、红色垫片站位、前移 `0.12 m` 与下降 `0.12 m`。
