...
FK/IK 规划器还会生成完整但默认禁止实机执行的动作序列：在检测到的夹取末端高度保持腕部法兰水平、夹爪竖直；夹爪张开后沿机器人基座 `+X` 前进到目标并闭合；沿 `+Z` 上提 `0.12 m`，沿 `+X` 向远端移动 `0.12 m`；第一段到此保持。第二段经独立授权后先沿 `-Z` 下降 `0.22 m`，随后沿 `-Z/-X` 斜向下降和内缩，松开夹爪，最后让夹爪指向轴朝地面系 `+X` 倾斜并同时沿 `-X` 内缩，使导热垫脱离。最终倾斜段的位置和姿态使用同步插值，每个中间点均请求 IK 并检查状态有效性。
...
用户明确指定的第一段上提/远移 `0.12/0.12 m` 和第二段下降 `0.22 m` 已固定；开放前进距离、斜向下降/内缩距离以及最终倾角尚无现场尺寸依据，目前仅为保守设计初值，并标记 `parameters_calibrated: false`。当前“向前倾斜”严格定义为夹爪/指尖的指向轴朝远离机器人的地面 `+X` 方向倾斜；它不代表末端位置向前平移。如果需要的是手腕上端向前、从而让指尖朝机器人内侧倾斜，则旋转方向必须反转。在该物理含义确认，以及完成 TCP、桌面 PlanningScene、夹持视觉验证和小步现场标定前，任何执行器都必须拒绝运行该放置/脱离序列。
